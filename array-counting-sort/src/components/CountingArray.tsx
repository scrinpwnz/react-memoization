                ...
                return (
                    <div>
                        <Paper className={cn(classes.item, {[classes.selected]: item.selected})} variant={"outlined"}>
                            <Typography variant={'h4'} color={'inherit'} align={'center'}>{item.value}</Typography>
                        </Paper>
                        <Typography variant={'h6'} color={'textSecondary'} align={'center'}>{index}</Typography>
                        <Spring
                            config={config.wobbly}
                            onResolve={() => prevData[index] = item.numberOfElements}
                            from={{height: prevData[index] ?? 0}}
                            to={{height: (item.numberOfElements) * 64 + ((item.numberOfElements > 1) ? (item.numberOfElements - 1) * 8 : 0)}}>
                            {props => <animated.div key={index} ref={item.ref} className={classes.spot}
                                                     style={props}/>}
                        </Spring>
                    </div>
                )
